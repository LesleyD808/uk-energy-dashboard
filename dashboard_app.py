...
# 领域提示（用于生成“可能原因”段落，作为启发而非绝对结论）
# 用 (fuel_name, "up"/"down") 做键；句子尽量 < 50 词
...
def _figure_for_pdf(fig: go.Figure, width=900, height=520, scale=2) -> bytes:
    """把图改成白底黑字并导出 PNG（给 ReportLab 用）"""
    fig = fig.to_dict()  # 断开引用，避免影响屏幕图
    fig = go.Figure(fig)
    fig.update_layout(
...
def build_pdf_report(summary_df, sector, year_a, year_b, change_bar_png: bytes):
    ...
    if 'BodySmall' not in styles.byName:
        styles.add(ParagraphStyle(
            name='BodySmall',
            parent=styles['BodyText'],   # 继承内置 BodyText
            fontSize=10,
            leading=14
        ))
    ...
    story.append(Paragraph(
        ...
        styles['BodySmall']           # ← 这里改成 BodySmall
    ))
...
if year_a != year_b:
    # 复用我们在 Compare Two Years 已经算好的 df_compare，或者重新计算一次更稳：
    compare_df = compute_compare_summary(df, selected_sector, year_a, year_b)

    # 可选：再画一个“Change % by fuel”的图，用于嵌入报告
    fig_change = make_change_bar(compare_df, selected_sector, year_a, year_b)  # 这个还是给屏幕看
    change_png = _figure_for_pdf(fig_change)  # ✅ 这个导出白底版给 PDF
